...
type StateRow = { id: number; data: unknown; version: number; updated_at: string };

export async function GET(request: Request) {
  const session = await sessionFromRequest(request);
  if (!session) return Response.json({ ok: false, error: "الجلسة منتهية" }, { status: 401 });
  try {
    const rows = await supabaseQuery<StateRow>("center_state", { select: "id,data,version,updated_at", id: "eq.1", limit: 1 });
    const row = rows[0];
    return Response.json({ ok: true, state: row?.data ?? emptyCenterState, version: row?.version ?? 0, updatedAt: row?.updated_at ?? null }, { headers: { "Cache-Control": "no-store" } });
  } catch {
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    const payload = { id: 1, data: body.state, version: nextVersion, updated_at: new Date().toISOString() };
    let saved: StateRow[];
    if (current) {
    ...
    if (!saved.length) {
      const latestRows = await supabaseQuery<StateRow>("center_state", { select: "id,data,version,updated_at", id: "eq.1", limit: 1 });
      const latest = latestRows[0];
      return Response.json({ ok: false, error: "تم تعديل البيانات من جهاز آخر", conflict: true, state: latest?.data ?? emptyCenterState, version: latest?.version ?? 0 }, { status: 409 });
    }
    return Response.json({ ok: true, version: saved[0]?.version ?? nextVersion, updatedAt: saved[0]?.updated_at ?? new Date().toISOString() }, { headers: { "Cache-Control": "no-store" } });
  } catch {
