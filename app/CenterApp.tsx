...
export default function CenterApp() {
  ...
  useEffect(() => {
    ...
    fetch("/api/auth/reception", { method: "POST", cache: "no-store" })
      ...
      .catch(() => { if (!cancelled) setAuthenticated(true); })
  ...
  useEffect(() => {
    ...
    const snapshot: CenterSnapshot = { students, teachers, pricing, sessions, bookings, expenses, debtPayments, audit, subjectCatalog, rooms, savedAt: new Date().toISOString() };
  ...
  const handleAdminPin = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setAdminAuthError("");
    try {
      const response = await fetch("/api/auth/login", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ password: data.get("pin") }) });
  ...
  const handleAdminPinRecovery = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setAdminAuthError("");
    try {
      const response = await fetch("/api/auth/recover", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username: currentUsername, recoveryCode: data.get("recoveryCode"), password: data.get("pin") }) });
      const result = await response.json() as { ok?: boolean; error?: string; username?: string };
    ...
    <div className="app-shell" dir="rtl">
      <aside className={`sidebar ${mobileNav ? "mobile-open" : ""}`}>
        ...
        <div className="sidebar-bottom">
          <div className="admin-menu-wrap"><div className="admin-mini"><span>م</span><div><strong>مدير السنتر</strong><small>{adminUnlocked ? currentUsername : "الإدارة مقفولة"}</small></div><button className="admin-more" onClick={() => setProfileMenuOpen((open) => !open)} aria-label="فتح قائمة مدير السنتر" aria-expanded={profileMenuOpen}><MoreHorizontal size={18} /></button></div>{profileMenuOpen && <div className="admin-popover" role="menu"><button role="menuitem" onClick={() => openAdmin("settings")}><Settings size={17} /><span><strong>إعدادات الإدارة</strong><small>تغيير اسم المستخدم أو PIN</small></span></button><button role="menuitem" onClick={() => openAdmin("audit")}><History size={17} /><span><strong>سجل العمليات</strong><small>عرض آخر التغييرات</small></span></button>{adminUnlocked && <button className="danger" role="menuitem" onClick={lockAdmin}><LockKeyhole size={17} /><span><strong>قفل الإدارة</strong><small>العودة لوضع الريسبشن</small></span></button>}</div>}</div>
          {adminUnlocked && <button className="logout" onClick={lockAdmin}><LockKeyhole size={18} /> قفل الإدارة</button>}
        </div>
      </aside>
      ...
      {adminPinOpen && <Modal title={adminRecoveryMode ? "استرداد PIN الإدارة" : "فتح صفحة الإدارة"} subtitle="باقي النظام متاح للريسبشن بدون كلمة مرور" onClose={() => { setAdminPinOpen(false); setAdminRecoveryMode(false); setAdminAuthError(""); }}>
        {!adminRecoveryMode ? <form className="modal-body admin-pin-form" onSubmit={handleAdminPin}><div className="admin-pin-icon"><LockKeyhole size={25} /></div><label className="field">PIN الإدارة — 4 أرقام<input name="pin" type="password" inputMode="numeric" autoComplete="current-password" minLength={4} maxLength={4} pattern="[0-9]{4}" autoFocus required placeholder="••••" /></label>{adminAuthError && <div className="form-error">{adminAuthError}</div>}<button className="primary-btn" type="submit" disabled={adminAuthLoading}>{adminAuthLoading ? <><LoaderCircle className="spin" size={17} /> جاري الفتح…</> : <>فتح الإدارة <ChevronLeft size={17} /></>}</button><button className="forgot-password" type="button" onClick={() => { setAdminAuthError(""); setAdminRecoveryMode(true); }}>نسيت PIN الإدارة؟</button></form> : <form className="modal-body admin-pin-form" onSubmit={handleAdminPinRecovery}><div className="admin-pin-icon recovery"><ShieldCheck size={25} /></div><label className="field">كود الطوارئ الثابت<input name="recoveryCode" autoComplete="off" required placeholder="XXXX-XXXX-XXXX-XXXX-XXXX" /></label><label className="field">PIN جديد — 4 أرقام<input name="pin" type="password" inputMode="numeric" autoComplete="new-password" minLength={4} maxLength={4} pattern="[0-9]{4}" required placeholder="••••" /></label>{adminAuthError && <div className="form-error">{adminAuthError}</div>}<button className="primary-btn" type="submit" disabled={adminAuthLoading}>{adminAuthLoading ? "جاري الاسترداد…" : "تعيين PIN وفتح الإدارة"}</button><button className="forgot-password" type="button" onClick={() => { setAdminAuthError(""); setAdminRecoveryMode(false); }}>العودة لإدخال PIN</button></form>}
      </Modal>}
...
function StudentsPage({ tab, setTab, students, setStudents, teachers, bookings, setBookings, sessions, debtPayments, setDebtPayments, onOpenStudent, audit, showToast }: { tab: StudentTab; setTab: (tab: StudentTab) => void; students: Student[]; setStudents: React.Dispatch<React.SetStateAction<Student[]>>; teachers: Teacher[]; bookings: Booking[]; setBookings: React.Dispatch<React.SetStateAction<Booking[]>>; sessions: LessonSession[]; debtPayments: DebtPayment[]; setDebtPayments: React.Dispatch<React.SetStateAction<DebtPayment[]>>; onOpenStudent: (student: Student) => void; audit: React.Dispatch<React.SetStateAction<AuditEntry[]>>; showToast: (message: string) => void }) {
  ...
  const [settlingDebt, setSettlingDebt] = useState<{ studentId: string; sessionId: string } | null>(null);
  ...
  const addStudent = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const data = new FormData(event.currentTarget); const newStudent: Student = { id: nextStudentIdForStage(students.map((student) => student.id), registerStage), name: String(data.get("name")), phone: String(data.get("phone")), stage: registerStage, grade: String(data.get("grade")), active: true }; setStudents((current) => [...current, newStudent]); audit((current) => [{ id: String(Date.now()), action: "إضافة طالب", details: `تم تسجيل ${newStudent.name} — ${newStudent.id}`, time: "الآن", tone: "blue" }, ...current]); event.currentTarget.reset(); showToast(`تم تسجيل الطالب بالرقم ${newStudent.id}`); };
  return <div className="stack-page"><div className="segmented-tabs"><button className={tab === "register" ? "active" : ""} onClick={() => setTab("register")}><UserPlus size={18} /> تسجيل طالب لأول مرة</button><button className={tab === "bookings" ? "active" : ""} onClick={() => setTab("bookings")}><BookOpen size={18} /> الحجز المسبق</button><button className={tab === "records" ? "active" : ""} onClick={() => setTab("records")}><FileClock size={18} /> سجل الطلاب</button><button className={tab === "debts" ? "active" : ""} onClick={() => setTab("debts")}><WalletCards size={18} /> طلاب عليهم مبالغ</button></div>
    {tab === "register" && <section className="split-layout"><div className="form-panel"><div className="form-panel-head"><span><UserPlus size={22} /></span><div><h2>تسجيل طالب جديد</h2><p>أدخل البيانات الأساسية، وسيتم إنشاء ID من 6 أرقام حسب المرحلة</p></div></div><form className="entity-form" onSubmit={addStudent}><label className="field full">اسم الطالب بالكامل<input name="name" required placeholder="مثال: أحمد محمد علي" /></label><label className="field full">رقم الهاتف<input name="phone" required inputMode="tel" placeholder="01xxxxxxxxx" /></label><label className="field">المرحلة<select value={registerStage} onChange={(event) => setRegisterStage(event.target.value as Stage)}>{stages.map((stage) => <option key={stage}>{stage}</option>)}</select></label><label className="field">الصف<select name="grade" required>{gradesByStage[registerStage].map((grade) => <option key={grade}>{grade}</option>)}</select></label><div className="student-id-preview full"><span>رقم الطالب المتوقع</span><strong>{nextStudentIdForStage(students.map((student) => student.id), registerStage)}</strong><small>{registerStage === "المرحلة الابتدائية" ? "10" : registerStage === "المرحلة الإعدادية" ? "20" : "30"} يحدد المرحلة · آخر 4 أرقام كود الطالب</small></div><div className="info-note full"><ShieldCheck size={18} /><span>يمكن استخدام نفس رقم الهاتف لأكثر من طالب، والـID الرقمي المكوّن من 6 أرقام هو المعرف الأساسي.</span></div><button className="primary-btn full" type="submit"><Plus size={18} /> تسجيل الطالب</button></form></div><div className="tip-card"><span><Sparkles size={25} /></span><h3>تسجيل سريع وواضح</h3><p>بعد التسجيل سيظهر الطالب فوراً في السجل، ويمكن البحث عنه وإضافته لأي حصة أو حجز مسبق.</p><div><Check size={17} /> ID حسب المرحلة</div><div><Check size={17} /> تعديل وأرشفة</div><div><Check size={17} /> سجل حضور كامل</div></div></section>}
    ...
    {tab === "debts" && <><section className="panel data-panel debt-panel"><div className="data-toolbar"><div><h2>الطلاب الذين عليهم مبالغ</h2><p>{studentsWithDebt.length} طالب · التحصيل هنا مستقل عن سعر أي حصة جديدة</p></div><div className="search-box"><Search size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ابحث بالاسم، الهاتف أو ID" /></div></div>{studentsWithDebt.length ? <div className="table-wrap"><table><thead><tr><th>الطالب</th><th>ID</th><th>عدد الحصص الناقصة</th><th>إجمالي المتبقي</th><th>آخر حصة ناقصة</th><th>إجراء</th></tr></thead><tbody>{studentsWithDebt.map((student) => { const debtSessions = debtsForStudent(student.id); const totalDebt = outstandingForStudent(sessions, debtPayments, student.id); const latest = debtSessions[0]; return <tr key={student.id}><td><div className="person-cell student"><span>{student.name.charAt(0)}</span><strong>{student.name}</strong></div><small>{student.phone}</small></td><td><code>{student.id}</code></td><td>{debtSessions.length} حصة</td><td><span className="debt-amount">{money(totalDebt)}</span></td><td><strong>{latest?.subject}</strong><small>{latest?.date}</small></td><td><div className="table-actions"><button onClick={() => onOpenStudent(student)} title="فتح سجل الطالب"><FileClock size={17} /></button><button className="settle-debt-btn" onClick={() => latest && setSettlingDebt({ studentId: student.id, sessionId: latest.id })}><WalletCards size={17} /> تسجيل سداد</button></div></td></tr>; })}</tbody></table></div> : <EmptyState icon={<WalletCards />} title="لا توجد مديونيات حالية" text="كل الطلاب مسددون قيمة حصصهم بالكامل" />}</section>{settlingDebt && selectedDebtStudent && selectedDebtSession && <Modal title="تسجيل سداد مديونية" subtitle={`${selectedDebtStudent.name} — ID ${selectedDebtStudent.id}`} onClose={() => setSettlingDebt(null)}><form className="modal-body entity-form" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); const amount = Math.min(selectedDebtRemaining, Math.max(0, Number(data.get("amount")))); if (!amount) { showToast("أدخل مبلغ سداد صحيح"); return; } const payment: DebtPayment = { id: String(Math.max(...debtPayments.map((item) => Number(item.id)), 0) + 1), studentId: selectedDebtStudent.id, sessionId: selectedDebtSession.id, amount, date: todayIso(), note: String(data.get("note") ?? "") }; setDebtPayments((current) => [payment, ...current]); audit((current) => [{ id: String(Date.now()), action: "سداد مديونية طالب", details: `تم تحصيل ${money(amount)} من ${selectedDebtStudent.name} عن حصة ${selectedDebtSession.subject} بدون تعديل سعر أي حصة جديدة`, time: "الآن", tone: "green" }, ...current]); setSettlingDebt(null); showToast(amount === selectedDebtRemaining ? "تم سداد المبلغ المتبقي بالكامل" : "تم تسجيل السداد الجزئي وتحديث المتبقي"); }}><label className="field full">الحصة المرتبطة بالمبلغ<select value={settlingDebt.sessionId} onChange={(event) => setSettlingDebt({ ...settlingDebt, sessionId: event.target.value })}>{selectedDebtSessions.map((lesson) => <option key={lesson.id} value={lesson.id}>{lesson.date} — {lesson.subject} — متبقي {money(outstandingForAttendance(lesson, selectedDebtStudent.id, debtPayments))}</option>)}</select></label><div className="debt-settlement-summary full"><span>المبلغ المتبقي على الحصة</span><strong>{money(selectedDebtRemaining)}</strong><small>هذا السداد يخص الحصة القديمة فقط، ولا يغيّر حساب الحصة التالية أو مستحق المدرس.</small></div><label className="field full">المبلغ المدفوع الآن<input name="amount" type="number" min="0.01" max={selectedDebtRemaining} step="0.01" defaultValue={selectedDebtRemaining} key={`${selectedDebtSession.id}-${selectedDebtRemaining}`} required /></label><label className="field full">ملاحظة (اختياري)<input name="note" placeholder="مثال: سداد باقي حصة يوم الخميس" /></label><button className="primary-btn full" type="submit">تأكيد السداد</button></form></Modal>}</>}
...
function BookingsPanel({ students, teachers, bookings, setBookings, showToast }: { students: Student[]; teachers: Teacher[]; bookings: Booking[]; setBookings: React.Dispatch<React.SetStateAction<Booking[]>>; showToast: (message: string) => void }) {
  ...
  const active = bookings.filter((booking) => booking.active).filter((booking) => { const student = students.find((item) => item.id === booking.studentId); const teacher = teachers.find((item) => item.id === booking.teacherId); return !normalized || `${student?.name ?? ""} ${student?.phone ?? ""} ${student?.id ?? ""} ${teacher?.name ?? ""} ${booking.subject} ${booking.grade}`.toLocaleLowerCase("ar").includes(normalized); }).slice().sort((left, right) => right.createdAt.localeCompare(left.createdAt) || newestNumericIdFirst(left, right));
  ...
  const closeBooking = () => { setOpen(false); setStudentQuery(""); setSelectedStudentId(""); };
  return <section className="panel data-panel"><div className="data-toolbar"><div><h2>الحجوزات المسبقة</h2><p>ربط الطالب بالمدرس وتسجيل قيمة الحجز بعيداً عن حساب الحصص</p></div><div className="toolbar-actions"><div className="search-box"><Search size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ابحث بالطالب، الهاتف، المدرس أو المادة" /></div><button className="primary-btn" onClick={() => setOpen(true)}><Plus size={18} /> حجز جديد</button></div></div><div className="booking-grid">{active.map((booking) => { const student = students.find((item) => item.id === booking.studentId); const teacher = teachers.find((item) => item.id === booking.teacherId); return <article key={booking.id} className="booking-card"><div className="booking-top"><span className="student-avatar">{student?.name.charAt(0)}</span><div><strong>{student?.name}</strong><small>{student?.id} · {booking.createdAt}</small></div><button onClick={() => { setBookings((current) => current.map((item) => item.id === booking.id ? { ...item, active: false } : item)); showToast("تم نقل الحجز للأرشيف"); }} aria-label="أرشفة الحجز"><Archive size={17} /></button></div><div className="booking-link"><span><GraduationCap size={17} /> {teacher?.name}</span><span><BookOpen size={17} /> {booking.subject}</span><span>{booking.stage} · {booking.grade}</span></div><div className="booking-fee"><span>قيمة الحجز</span><strong>{money(booking.bookingFee)}</strong></div></article>; })}</div>{!active.length && <EmptyState icon={<BookOpen />} title="لا توجد حجوزات مطابقة" text={query ? "غيّر كلمات البحث لعرض حجوزات أخرى" : "ابدأ بربط طالب مع المدرس المناسب"} />}
    {open && <Modal title="حجز مسبق جديد" subtitle="ابحث عن الطالب ثم اربطه بالمدرس والمادة" onClose={closeBooking}><form className="modal-body entity-form" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); if (!selectedTeacher || !selectedAssignment || !selectedStudentId) { showToast("ابحث عن الطالب واختره أولاً"); return; } setBookings((current) => [...current, { id: String(Math.max(...current.map((booking) => Number(booking.id)), 0) + 1), studentId: selectedStudentId, teacherId: selectedTeacher.id, stage: selectedAssignment.stage, grade: selectedAssignment.grade, subject: selectedAssignment.subject, bookingFee: Number(data.get("bookingFee")), createdAt: todayIso(), active: true }]); closeBooking(); showToast("تم إنشاء الحجز وتسجيل قيمته"); }}><div className="field full booking-student-search"><span>الطالب</span>{selectedStudent ? <div className="selected-student-chip"><span>{selectedStudent.name.charAt(0)}</span><div><strong>{selectedStudent.name}</strong><small>{selectedStudent.id} · {selectedStudent.phone}</small></div><button type="button" onClick={() => { setSelectedStudentId(""); setStudentQuery(""); }} aria-label="تغيير الطالب"><X size={16} /></button></div> : <><div className="search-box large"><Search size={18} /><input value={studentQuery} onChange={(event) => setStudentQuery(event.target.value)} autoFocus placeholder="ابحث بالاسم، الهاتف أو ID" /></div>{studentCandidates.length > 0 && <div className="booking-student-results">{studentCandidates.map((student) => <button type="button" key={student.id} onClick={() => { setSelectedStudentId(student.id); setStudentQuery(student.name); }}><span>{student.name.charAt(0)}</span><div><strong>{student.name}</strong><small>{student.id} · {student.phone}</small></div><ChevronLeft size={16} /></button>)}</div>}</>}</div><label className="field full">المدرس<select value={teacherId} onChange={(event) => { setTeacherId(event.target.value); setAssignmentIndex(0); }}>{teachers.filter((teacher) => teacher.active).map((teacher) => <option key={teacher.id} value={teacher.id}>{teacher.name}</option>)}</select></label><label className="field full">المرحلة والصف والمادة<select value={assignmentIndex} onChange={(event) => setAssignmentIndex(Number(event.target.value))}>{selectedTeacher?.assignments.map((assignment, index) => <option value={index} key={`${assignment.stage}-${assignment.grade}-${assignment.subject}`}>{assignment.stage} — {assignment.grade} — {assignment.subject}</option>)}</select></label><label className="field full">قيمة الحجز<input name="bookingFee" type="number" min="0" required placeholder="مثال: 200" /></label><div className="info-note full"><CircleDollarSign size={18} /> قيمة الحجز مستقلة عن الحصص ولا تدخل في حساب مستحق المدرس أو صافي الحصة.</div><button className="primary-btn full" type="submit">تأكيد الحجز</button></form></Modal>}
  </section>;
}
...
function TeachersPage({ teachers, setTeachers, sessions, onOpenTeacher, audit, subjectCatalog, setSubjectCatalog, showToast }: { teachers: Teacher[]; setTeachers: React.Dispatch<React.SetStateAction<Teacher[]>>; sessions: LessonSession[]; onOpenTeacher: (teacher: Teacher) => void; audit: React.Dispatch<React.SetStateAction<AuditEntry[]>>; subjectCatalog: Record<Stage, string[]>; setSubjectCatalog: React.Dispatch<React.SetStateAction<Record<Stage, string[]>>>; showToast: (message: string) => void }) {
  ...
  const archiveTeacher = (teacher: Teacher) => {
    ...
    audit((current) => [{ id: String(Date.now()), action: "أرشفة مدرس", details: `تمت أرشفة ${teacher.name} مع الاحتفاظ بكل حصصه وحساباته`, time: "الآن", tone: "orange" }, ...current]);
    setOpenTeacherMenuId(null);
    showToast("تم نقل المدرس للأرشيف مع الاحتفاظ بحصصه وحساباته");
  };
  return <div className="stack-page"><section className="panel data-panel"><div className="data-toolbar"><div><h2>قائمة المدرسين</h2><p>{visible.length} مدرس نشط</p></div><div className="toolbar-actions"><div className="search-box"><Search size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ابحث عن مدرس" /></div><button className="primary-btn" onClick={() => openForm()}><Plus size={18} /> إضافة مدرس</button></div></div><div className="teacher-grid">{visible.map((teacher) => { const teacherSessions = sessions.filter((lesson) => lesson.teacherId === teacher.id && lesson.status === "ended"); return <article className="teacher-card" key={teacher.id}><div className="teacher-card-head"><span>{teacher.name.replace("أ/ ", "").charAt(0)}</span><div><h3>{teacher.name}</h3><small>{teacher.id} · {teacher.phone}</small></div><div className="teacher-menu-wrap"><button type="button" className="teacher-more" onClick={() => setOpenTeacherMenuId((current) => current === teacher.id ? null : teacher.id)} aria-label={`فتح إجراءات ${teacher.name}`} aria-expanded={openTeacherMenuId === teacher.id}><MoreHorizontal size={19} /></button>{openTeacherMenuId === teacher.id && <div className="teacher-card-menu"><button type="button" onClick={() => { setOpenTeacherMenuId(null); onOpenTeacher(teacher); }}><History size={16} /> سجل الحصص</button><button type="button" onClick={() => { setOpenTeacherMenuId(null); openForm(teacher); }}><SquarePen size={16} /> تعديل البيانات</button><button type="button" className="danger" onClick={() => archiveTeacher(teacher)}><Archive size={16} /> نقل للأرشيف</button></div>}</div></div><div className="assignment-tags">{teacher.assignments.map((assignment) => <span key={`${assignment.stage}-${assignment.grade}-${assignment.subject}`}>{assignment.subject}<small>{assignment.stage.replace("المرحلة ", "")} · {assignment.grade.replace("الصف ", "")}</small></span>)}</div><div className="teacher-stats"><div><strong>{teacherSessions.length}</strong><span>حصة منتهية</span></div><div><strong>{teacherSessions.reduce((sum, lesson) => sum + lesson.studentIds.length, 0)}</strong><span>حضور طالب</span></div></div><div className="teacher-actions"><button onClick={() => onOpenTeacher(teacher)}><History size={17} /> سجل الحصص</button><button onClick={() => openForm(teacher)}><SquarePen size={17} /> تعديل</button><button className="archive-action" onClick={() => archiveTeacher(teacher)} aria-label={`نقل ${teacher.name} للأرشيف`}><Archive size={17} /></button></div></article>; })}</div></section>
...
function ExpensesPage({ expenses, setExpenses, audit, showToast }: { expenses: CenterExpense[]; setExpenses: React.Dispatch<React.SetStateAction<CenterExpense[]>>; audit: React.Dispatch<React.SetStateAction<AuditEntry[]>>; showToast: (message: string) => void }) {
  ...
  const normalized = query.trim().toLocaleLowerCase("ar");
  const visible = expenses.filter((expense) => category === "all" || expense.category === category).filter((expense) => !normalized || `${expense.description} ${expense.category} ${expense.date} ${expense.id}`.toLocaleLowerCase("ar").includes(normalized)).sort((a, b) => b.date.localeCompare(a.date) || Number(b.id) - Number(a.id));
  const total = visible.reduce((sum, expense) => sum + expense.amount, 0);
  return <div className="expenses-stack"><section className="expenses-hero"><div><span className="eyebrow"><ReceiptText size={16} /> حسابات السنتر</span><h2>كل مصروف<br />متسجّل ومحسوب.</h2><p>سجّل مصروفات التشغيل، وسيتم خصمها تلقائيًا من صافي ربح السنتر في الإحصائيات.</p></div><div className="expense-total-card"><span>إجمالي النتائج الحالية</span><strong>{money(total)}</strong><small>{visible.length} مصروف</small></div><button className="primary-btn" onClick={() => setOpen(true)}><Plus size={18} /> إضافة مصروف</button></section><section className="panel data-panel"><div className="data-toolbar expense-toolbar"><div><h2>سجل المصروفات</h2><p>{visible.length} سجل · {money(total)}</p></div><div className="expense-filter-actions"><div className="search-box"><Search size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ابحث في الوصف أو التاريخ أو ID" /></div><label className="expense-category-filter"><select value={category} onChange={(event) => setCategory(event.target.value as "all" | CenterExpense["category"])}><option value="all">كل الفئات</option>{categories.map((item) => <option key={item}>{item}</option>)}</select></label></div></div>{visible.length ? <div className="table-wrap"><table><thead><tr><th>ID</th><th>التاريخ</th><th>الفئة</th><th>الوصف</th><th>المبلغ</th><th>إجراءات</th></tr></thead><tbody>{visible.map((expense) => <tr key={expense.id}><td><code>{expense.id}</code></td><td>{expense.date}</td><td><span className="expense-category-tag">{expense.category}</span></td><td><strong>{expense.description}</strong></td><td><span className="expense-amount">{money(expense.amount)}</span></td><td><button type="button" className="table-icon delete" onClick={() => setDeletingExpense(expense)} aria-label={`حذف مصروف ${expense.description}`}><Trash2 size={17} /></button></td></tr>)}</tbody><tfoot><tr><td colSpan={4}>إجمالي المصروفات المعروضة</td><td colSpan={2}>{money(total)}</td></tr></tfoot></table></div> : <EmptyState icon={<ReceiptText />} title="لا توجد مصروفات" text="أضف أول مصروف ليظهر في السجل والإحصائيات" />}</section>{open && <Modal title="إضافة مصروف جديد" subtitle="سيُخصم المبلغ من صافي ربح السنتر حسب تاريخ المصروف" onClose={() => setOpen(false)}><form className="modal-body entity-form" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); const expense: CenterExpense = { id: String(Math.max(...expenses.map((item) => Number(item.id)), 0) + 1), category: String(data.get("category")) as CenterExpense["category"], amount: Number(data.get("amount")), date: String(data.get("date")), description: String(data.get("description")).trim() }; setExpenses((current) => [expense, ...current]); audit((current) => [{ id: String(Date.now()), action: "إضافة مصروف", details: `${expense.description} — ${money(expense.amount)}`, time: "الآن", tone: "orange" }, ...current]); setOpen(false); showToast("تم تسجيل المصروف وخصمه من الحسابات"); }}><label className="field">الفئة<select name="category">{categories.map((item) => <option key={item}>{item}</option>)}</select></label><label className="field">التاريخ<input name="date" type="date" defaultValue={todayIso()} required /></label><label className="field full">الوصف<input name="description" placeholder="مثال: فاتورة الكهرباء" required /></label><label className="field full">المبلغ<input name="amount" type="number" min="0.01" step="0.01" required /></label><button className="primary-btn full" type="submit">حفظ المصروف</button></form></Modal>}{deletingExpense && <Modal title="حذف المصروف" subtitle="سيتم تعديل صافي الربح بعد الحذف" onClose={() => setDeletingExpense(null)}><div className="modal-body"><div className="delete-review"><Trash2 size={22} /><div><strong>{deletingExpense.description}</strong><span>{deletingExpense.date} · {deletingExpense.category} · {money(deletingExpense.amount)}</span></div></div></div><div className="modal-actions"><button className="secondary-btn" onClick={() => setDeletingExpense(null)}>إلغاء</button><button className="danger-confirm" onClick={() => { const removed = deletingExpense; setExpenses((current) => current.filter((expense) => expense.id !== removed.id)); audit((current) => [{ id: String(Date.now()), action: "حذف مصروف", details: `${removed.description} — ${money(removed.amount)}`, time: "الآن", tone: "orange" }, ...current]); setDeletingExpense(null); showToast("تم حذف المصروف وتحديث الحسابات"); }}>تأكيد الحذف</button></div></Modal>}</div>;
}
  ...
  return <>
    <Modal title={`${session.subject} — ${gradeLabel(session.stage, session.grade)}`} subtitle={`${session.id} · ${session.room}`} onClose={onClose} wide>
      <div className="session-modal-body">
        ...
        {session.status !== "ended" && <div className="student-search-wrap"><label>إضافة طالب للحصة</label><div className="search-box large"><Search size={19} /><input value={query} onChange={(event) => { setQuery(event.target.value); setStudentError(""); }} placeholder="ابحث بالاسم، رقم الهاتف أو ID" /></div>{studentError && <div className="student-conflict-alert"><Users size={17} /> {studentError}</div>}{candidates.length > 0 && <div className="search-results">{candidates.map((student) => { const oldDebt = outstandingForStudent(allSessions, debtPayments, student.id); const conflict = session.status === "active" ? findActiveStudentConflict(allSessions, session.id, student.id) : undefined; return <button key={student.id} className={conflict ? "student-busy" : ""} onClick={() => openPayment(student)}><span>{student.name.charAt(0)}</span><div><strong>{student.name}</strong><small>{student.id} · {student.phone}</small>{oldDebt > 0 && <em className="candidate-debt">عليه سابقاً {money(oldDebt)}</em>}{conflict && <em className="candidate-conflict">داخل حصة {conflict.subject} · {conflict.room}</em>}</div>{conflict ? <X size={18} /> : <Plus size={18} />}</button>; })}</div>}</div>}
...
function StudentRecordModal({ student, sessions, teachers, debtPayments, onClose }: { student: Student; sessions: LessonSession[]; teachers: Teacher[]; debtPayments: DebtPayment[]; onClose: () => void }) {
  ...
  return <Modal title="سجل الطالب" subtitle={`${student.name} — ${student.id}`} onClose={onClose} wide><div className="modal-body"><div className="profile-strip student-finance-strip"><span>{student.name.charAt(0)}</span><div><h3>{student.name}</h3><p>{student.phone} · {gradeLabel(student.stage, student.grade)}</p></div><div><strong>{history.length}</strong><small>حصة مكتملة</small></div><div><strong>{money(totalPaid)}</strong><small>إجمالي ما تم دفعه</small></div><div className={outstanding ? "student-debt-total" : ""}><strong>{money(outstanding)}</strong><small>إجمالي المتبقي عليه</small></div></div><div className="timeline">{history.map((lesson) => { const paid = paidDuringSession(lesson, student.id); const initialShortage = shortageForAttendance(lesson, student.id); const remaining = outstandingForAttendance(lesson, student.id, debtPayments); const settled = Math.max(0, initialShortage - remaining); return <article key={lesson.id}><i /><div className="timeline-time"><strong>{lesson.date}</strong><span>{lesson.startedAt} — {lesson.endedAt}</span></div><div className="timeline-card student-payment-card"><div><h4>{lesson.subject}</h4><p>{teachers.find((teacher) => teacher.id === lesson.teacherId)?.name ?? "مدرس مؤرشف"} · {gradeLabel(lesson.stage, lesson.grade)} · {lesson.room}</p><small>سعر الحصة {money(lesson.studentPrice)} · دفع وقت الحصة {money(paid)}{settled > 0 ? ` · سداد لاحق ${money(settled)}` : ""}</small></div><strong className={remaining ? "debt-text" : ""}>{remaining ? `متبقي ${money(remaining)}` : "مسدد"}</strong></div></article>; })}</div>{!history.length && <EmptyState icon={<FileClock />} title="لا يوجد سجل حتى الآن" text="ستظهر حصص الطالب المنتهية هنا" />}</div></Modal>;
}
...
function ArchivePanel({ sessions, debtPayments, students, teachers }: { sessions: LessonSession[]; debtPayments: DebtPayment[]; students: Student[]; teachers: Teacher[] }) {
  ...
  return <><section className="panel data-panel"><div className="data-toolbar archive-toolbar"><div><h2>أرشيف الحصص</h2><p>{history.length} حصة مطابقة · اضغط على أي حصة لعرض معلوماتها</p></div><div className="archive-filters"><div className="search-box"><Search size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="اليوم، اسم المدرس أو المادة" /></div><label className="archive-date"><CalendarDays size={17} /><input type="date" value={dateFilter} onInput={(event) => setDateFilter(event.currentTarget.value)} aria-label="فلترة بتاريخ الحصة" /></label>{(query || dateFilter) && <button className="clear-filters" onClick={() => { setQuery(""); setDateFilter(""); }}><X size={16} /> مسح</button>}</div></div>{history.length ? <div className="archive-list detailed">{history.map((lesson) => { const financials = getSessionFinancials(lesson); return <button type="button" className="archive-row" key={lesson.id} onClick={() => setSelectedLesson(lesson)} aria-label={`فتح تفاصيل حصة ${lesson.subject}`}><StatusPill status="ended" /><div><strong>{lesson.subject} — {gradeLabel(lesson.stage, lesson.grade)}</strong><small>{teacherName(lesson.teacherId)} · {lesson.date} · {lesson.startedAt}–{lesson.endedAt}</small></div><span>{lesson.room}</span><span>{lesson.studentIds.length} طلاب</span><strong>{money(financials.collected)}</strong><b>{money(financials.centerNet)} صافي</b><ChevronLeft size={18} /></button>; })}</div> : <EmptyState icon={<Archive />} title="لا توجد حصص مطابقة" text="جرّب البحث باسم مدرس أو مادة، أو غيّر التاريخ" />}</section>{selectedLesson && <ArchivedSessionDetailsModal session={selectedLesson} debtPayments={debtPayments} students={students} teacherName={teacherName(selectedLesson.teacherId)} onClose={() => setSelectedLesson(null)} />}</>;
}
function ArchivedSessionDetailsModal({ session, debtPayments, students, teacherName, onClose }: { session: LessonSession; debtPayments: DebtPayment[]; students: Student[]; teacherName: string; onClose: () => void }) {
  ...
  return <Modal title="تفاصيل الحصة المؤرشفة" subtitle={`${session.id} · ${session.date} · ${session.room}`} onClose={onClose} wide><div className="modal-body archive-details"><div className="archive-detail-head"><div><StatusPill status="ended" /><h3>{session.subject} — {gradeLabel(session.stage, session.grade)}</h3><p>{teacherName} · من {session.startedAt} إلى {session.endedAt}</p></div><span><ShieldCheck size={18} /> سجل مالي محفوظ</span></div><div className="financial-grid session-financial-grid"><div><span>عدد الطلاب</span><strong>{session.studentIds.length}</strong></div><div><span>قيمة الحصة كاملة</span><strong>{money(financials.fullTotal)}</strong></div><div className={financials.shortages ? "shortage-card" : ""}><span>النواقص وقت الحصة</span><strong>{money(financials.shortages)}</strong></div><div><span>المحصل وقت الحصة</span><strong>{money(financials.collected)}</strong></div><div><span>مستحق المدرس</span><strong>{money(financials.teacherDue)}</strong></div><div className="highlight"><span>صافي السنتر وقتها</span><strong>{money(financials.centerNet)}</strong></div></div>{financials.shortages > 0 && <div className="archive-debt-note"><WalletCards size={19} /><span><strong>المتبقي حالياً من نواقص هذه الحصة: {money(outstanding)}</strong><small>أي سداد لاحق محفوظ كتحصيل مستقل ولا يغيّر مستحق المدرس أو سعر حصة أخرى.</small></span></div>}<div className="archive-unit-prices"><div><span>سعر الطالب</span><strong>{money(session.studentPrice)}</strong></div><div><span>أجر المدرس لكل طالب</span><strong>{money(session.teacherFee)}</strong></div><div><span>القاعة</span><strong>{session.room}</strong></div></div><div className="attendance-list"><div className="attendance-head"><h3>طلاب الحصة</h3><span>{session.studentIds.length} طالب</span></div>{session.studentIds.map((studentId, index) => { const student = students.find((item) => item.id === studentId); const paid = paidDuringSession(session, studentId); const remaining = outstandingForAttendance(session, studentId, debtPayments); return <div className="attendance-row" key={studentId}><span className="row-number">{index + 1}</span><span className="student-avatar">{student?.name.charAt(0) ?? "—"}</span><div><strong>{student?.name ?? "طالب مؤرشف"}</strong><small>{studentId} · {student?.phone ?? "لا يوجد رقم"}</small></div><span className={remaining ? "debt-badge" : "paid-tag"}>{remaining ? `متبقي ${money(remaining)}` : <><Check size={14} /> مسدد</>}</span><strong>{money(paid)}</strong></div>; })}</div></div></Modal>;
}
function AnalyticsPanel({ sessions, bookings, expenses, debtPayments, teachers }: { sessions: LessonSession[]; bookings: Booking[]; expenses: CenterExpense[]; debtPayments: DebtPayment[]; teachers: Teacher[] }) {
  ...
  const net = gross - teacherDue - expenseTotal;
  ...
  const bySubject = Array.from(new Set([...filtered.map((lesson) => lesson.subject), ...filteredBookings.map((booking) => booking.subject), ...recoveredSubjects])).map((subject) => { const lessons = filtered.filter((lesson) => lesson.subject === subject); const dimensionLessonIds = new Set(dimensionFiltered.filter((lesson) => lesson.subject === subject).map((lesson) => lesson.id)); return { label: subject, value: lessons.reduce((sum, lesson) => sum + getSessionFinancials(lesson).collected, 0) + recoveredForSessions(dimensionLessonIds) + filteredBookings.filter((booking) => booking.subject === subject).reduce((sum, booking) => sum + booking.bookingFee, 0) }; }).sort((a, b) => b.value - a.value);
  const byStage = stages.map((stage) => { const lessons = filtered.filter((lesson) => lesson.stage === stage); const dimensionLessonIds = new Set(dimensionFiltered.filter((lesson) => lesson.stage === stage).map((lesson) => lesson.id)); return { label: stage.replace("المرحلة ", ""), value: lessons.reduce((sum, lesson) => sum + getSessionFinancials(lesson).collected, 0) + recoveredForSessions(dimensionLessonIds) + filteredBookings.filter((booking) => booking.stage === stage).reduce((sum, booking) => sum + booking.bookingFee, 0) }; }).filter((item) => item.value > 0).sort((a, b) => b.value - a.value);
  const teacherRank = teachers.map((teacher) => { const lessons = filtered.filter((lesson) => lesson.teacherId === teacher.id); const dimensionLessonIds = new Set(dimensionFiltered.filter((lesson) => lesson.teacherId === teacher.id).map((lesson) => lesson.id)); return { teacher, sessions: lessons.length, bookings: filteredBookings.filter((booking) => booking.teacherId === teacher.id).length, attendance: lessons.reduce((sum, lesson) => sum + lesson.studentIds.length, 0), revenue: lessons.reduce((sum, lesson) => sum + getSessionFinancials(lesson).collected, 0) + recoveredForSessions(dimensionLessonIds) + filteredBookings.filter((booking) => booking.teacherId === teacher.id).reduce((sum, booking) => sum + booking.bookingFee, 0) }; }).filter((item) => item.sessions > 0 || item.bookings > 0 || item.revenue > 0).sort((a, b) => b.revenue - a.revenue);
  const monthlyMap = new Map<string, { key: string; label: string; sessions: number; bookings: number; attendance: number; gross: number; teacherDue: number; expenses: number; net: number }>();
  dimensionFiltered.forEach((lesson) => {
    const key = lesson.date.slice(0, 7);
    const current = monthlyMap.get(key) ?? { key, label: new Intl.DateTimeFormat("ar-EG", { month: "long", year: "numeric" }).format(new Date(`${key}-01T12:00:00`)), sessions: 0, bookings: 0, attendance: 0, gross: 0, teacherDue: 0, expenses: 0, net: 0 };
  ...
  debtPayments.filter((payment) => dimensionSessionIds.has(payment.sessionId)).forEach((payment) => {
...
function SettingsPanel({ subjectCatalog, setSubjectCatalog, rooms, currentUsername, onCredentialsChanged, onAddRoom, onRenameRoom, showToast }: { subjectCatalog: Record<Stage, string[]>; setSubjectCatalog: React.Dispatch<React.SetStateAction<Record<Stage, string[]>>>; rooms: string[]; currentUsername: string; onCredentialsChanged: (username: string) => void; onAddRoom: (room: string) => void; onRenameRoom: (index: number, room: string) => void; showToast: (message: string) => void }) {
  ...
  const saveRoom = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!value) { showToast("اكتب اسم القاعة أولاً"); return; }
    ...
    if (duplicate) { showToast("اسم القاعة موجود بالفعل"); return; }
    if (roomEditor.index === null) { onAddRoom(value); showToast("تمت إضافة القاعة"); }
