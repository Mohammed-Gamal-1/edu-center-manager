...
function AnalyticsPanel({ sessions, bookings, expenses, teachers }: { sessions: LessonSession[]; bookings: Booking[]; expenses: CenterExpense[]; teachers: Teacher[] }) {
  ...
  const sessionGross = filtered.reduce((sum, lesson) => sum + lesson.studentIds.length * lesson.studentPrice, 0);
  const bookingRevenue = filteredBookings.reduce((sum, booking) => sum + booking.bookingFee, 0);
  const gross = sessionGross + bookingRevenue;
  ...
  const bySubject = Array.from(new Set([...filtered.map((lesson) => lesson.subject), ...filteredBookings.map((booking) => booking.subject)])).map((subject) => ({ label: subject, value: filtered.filter((lesson) => lesson.subject === subject).reduce((sum, lesson) => sum + lesson.studentIds.length * lesson.studentPrice, 0) + filteredBookings.filter((booking) => booking.subject === subject).reduce((sum, booking) => sum + booking.bookingFee, 0) })).sort((a, b) => b.value - a.value);
  const byStage = stages.map((stage) => ({ label: stage.replace("المرحلة ", ""), value: filtered.filter((lesson) => lesson.stage === stage).reduce((sum, lesson) => sum + lesson.studentIds.length * lesson.studentPrice, 0) + filteredBookings.filter((booking) => booking.stage === stage).reduce((sum, booking) => sum + booking.bookingFee, 0) })).filter((item) => item.value > 0).sort((a, b) => b.value - a.value);
  const teacherRank = teachers.map((teacher) => ({ teacher, sessions: filtered.filter((lesson) => lesson.teacherId === teacher.id).length, bookings: filteredBookings.filter((booking) => booking.teacherId === teacher.id).length, attendance: filtered.filter((lesson) => lesson.teacherId === teacher.id).reduce((sum, lesson) => sum + lesson.studentIds.length, 0), revenue: filtered.filter((lesson) => lesson.teacherId === teacher.id).reduce((sum, lesson) => sum + lesson.studentIds.length * lesson.studentPrice, 0) + filteredBookings.filter((booking) => booking.teacherId === teacher.id).reduce((sum, booking) => sum + booking.bookingFee, 0) })).filter((item) => item.sessions > 0 || item.bookings > 0).sort((a, b) => b.revenue - a.revenue);
