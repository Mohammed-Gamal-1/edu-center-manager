...
const initialBookings: Booking[] = [
  { id: "301", studentId: "1042", teacherId: "18", stage: "المرحلة الإعدادية", grade: "الصف الثالث", subject: "الرياضيات", createdAt: todayIso(), active: true },
  { id: "302", studentId: "1043", teacherId: "24", stage: "المرحلة الثانوية", grade: "الصف الثاني", subject: "الفيزياء", createdAt: todayIso(), active: true },
];
    ...
    <div className="app-shell" dir="rtl">
      ...
      <main className="workspace">
        ...
        <div className="page-content">
          ...
          {view === "admin" && <AdminPage tab={adminTab} setTab={setAdminTab} pricing={pricing} setPricing={setPricing} sessions={sessions} teachers={teachers} audit={audit} subjectCatalog={subjectCatalog} setSubjectCatalog={setSubjectCatalog} showToast={showToast} />}
        </div>
      </main>
...
function BookingsPanel({ students, teachers, bookings, setBookings, showToast }: { students: Student[]; teachers: Teacher[]; bookings: Booking[]; setBookings: React.Dispatch<React.SetStateAction<Booking[]>>; showToast: (message: string) => void }) {
  ...
  return <section className="panel data-panel"><div className="data-toolbar"><div><h2>الحجوزات المسبقة</h2><p>ربط الطالب بالمدرس بعيداً عن الحصص والمدفوعات</p></div><button className="primary-btn" onClick={() => setOpen(true)}><Plus size={18} /> حجز جديد</button></div><div className="booking-grid">{active.map((booking) => { const student = students.find((item) => item.id === booking.studentId); const teacher = teachers.find((item) => item.id === booking.teacherId); return <article key={booking.id} className="booking-card"><div className="booking-top"><span className="student-avatar">{student?.name.charAt(0)}</span><div><strong>{student?.name}</strong><small>{student?.id}</small></div><button onClick={() => { setBookings((current) => current.map((item) => item.id === booking.id ? { ...item, active: false } : item)); showToast("تم نقل الحجز للأرشيف"); }}><Archive size={17} /></button></div><div className="booking-link"><span><GraduationCap size={17} /> {teacher?.name}</span><span><BookOpen size={17} /> {booking.subject}</span><span>{booking.stage} · {booking.grade}</span></div></article>; })}</div>{!active.length && <EmptyState icon={<BookOpen />} title="لا توجد حجوزات مسبقة" text="ابدأ بربط طالب مع المدرس المناسب" />}
    {open && <Modal title="حجز مسبق جديد" subtitle="الحجز لا يرتبط بحصة أو مدفوعات" onClose={() => setOpen(false)}><form className="modal-body entity-form" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); const teacher = teachers.find((item) => item.id === data.get("teacher")); const assignment = teacher?.assignments[0]; if (!teacher || !assignment) return; setBookings((current) => [...current, { id: String(Math.max(...current.map((booking) => Number(booking.id)), 0) + 1), studentId: String(data.get("student")), teacherId: teacher.id, stage: assignment.stage, grade: assignment.grade, subject: assignment.subject, createdAt: todayIso(), active: true }]); setOpen(false); showToast("تم إنشاء الحجز المسبق"); }}><label className="field full">الطالب<select name="student">{students.filter((student) => student.active).map((student) => <option key={student.id} value={student.id}>{student.name} — {student.id}</option>)}</select></label><label className="field full">المدرس<select name="teacher">{teachers.filter((teacher) => teacher.active).map((teacher) => <option key={teacher.id} value={teacher.id}>{teacher.name} — {teacher.assignments[0]?.stage} · {teacher.assignments[0]?.grade}</option>)}</select></label><div className="info-note full"><BookOpen size={18} /> سيتم ربط الحجز بأول مرحلة وصف ومادة للمدرس في الـDemo، ويمكن اختيار الربط بالتفصيل عند توصيل قاعدة البيانات.</div><button className="primary-btn full" type="submit">تأكيد الحجز</button></form></Modal>}
  </section>;
}
...
function AdminPage({ tab, setTab, pricing, setPricing, sessions, teachers, audit, subjectCatalog, setSubjectCatalog, showToast }: { tab: AdminTab; setTab: (tab: AdminTab) => void; pricing: PriceRule[]; setPricing: React.Dispatch<React.SetStateAction<PriceRule[]>>; sessions: LessonSession[]; teachers: Teacher[]; audit: AuditEntry[]; subjectCatalog: Record<Stage, string[]>; setSubjectCatalog: React.Dispatch<React.SetStateAction<Record<Stage, string[]>>>; showToast: (message: string) => void }) {
...
function AnalyticsPanel({ sessions, teachers }: { sessions: LessonSession[]; teachers: Teacher[] }) {
  ...
  const bySubject = Array.from(new Set(ended.map((lesson) => lesson.subject))).map((subject) => ({ subject, value: ended.filter((lesson) => lesson.subject === subject).reduce((sum, lesson) => sum + lesson.studentIds.length * lesson.studentPrice, 0) })).filter((item) => item.value > 0);
