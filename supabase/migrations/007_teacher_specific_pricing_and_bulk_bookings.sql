-- Make lesson pricing teacher-specific without changing historical session snapshots.
-- Existing stage/grade/subject prices are copied to every current teacher who
-- owns the matching assignment. Unmatched legacy prices remain nullable.
...
-- Attach each existing relational rule to the first matching teacher before
-- replacing the old grade/subject uniqueness rule.
with ranked_matches as (
  select rule.id as rule_id,
         assignment.teacher_id,
         row_number() over (partition by rule.id order by assignment.teacher_id) as match_order
  from public.price_rules as rule
  join public.teacher_assignments as assignment
    on assignment.grade_id = rule.grade_id
   and assignment.subject_id = rule.subject_id
...
set teacher_id = ranked_matches.teacher_id
from ranked_matches
where ranked_matches.rule_id = rule.id
  and ranked_matches.match_order = 1
  and rule.teacher_id is null;

drop index if exists public.one_active_price_per_grade_subject;

create unique index if not exists one_active_price_per_teacher_grade_subject
  on public.price_rules (teacher_id, grade_id, subject_id)
  where active = true and teacher_id is not null;

create unique index if not exists one_active_legacy_price_per_grade_subject
  on public.price_rules (grade_id, subject_id)
  where active = true and teacher_id is null;

-- Copy each pre-migration rule to the remaining teachers for that assignment.
-- The count guard makes a retry harmless after teacher-specific rows exist.
with legacy_sources as (
  select min(id::text)::uuid as rule_id
...
insert into public.price_rules (
  teacher_id, grade_id, subject_id, student_price,
  teacher_fee_per_student, effective_from, effective_to, active
)
select assignment.teacher_id,
       rule.grade_id,
       rule.subject_id,
       rule.student_price,
       rule.teacher_fee_per_student,
       rule.effective_from,
       rule.effective_to,
       rule.active
from public.price_rules as rule
join legacy_sources as source on source.rule_id = rule.id
join public.teacher_assignments as assignment
  on assignment.grade_id = rule.grade_id
 and assignment.subject_id = rule.subject_id
 and assignment.active = true
join public.teachers as teacher
 ...
 and teacher.active = true
where rule.active = true
on conflict (teacher_id, grade_id, subject_id) where active = true and teacher_id is not null do update
...
begin
  ...
  -- Price rules are teacher-specific while lesson sessions retain their own price snapshots.
  insert into public.price_rules (teacher_id, grade_id, subject_id, student_price, teacher_fee_per_student, effective_from, effective_to, active)
  select nullif(item ->> 'teacherId', '')::bigint,
         grade_row.id,
         subject_row.id,
  ...
  where nullif(item ->> 'teacherId', '') is not null
  on conflict (teacher_id, grade_id, subject_id) where active = true and teacher_id is not null do update
    set student_price = excluded.student_price,
        teacher_fee_per_student = excluded.teacher_fee_per_student,
        effective_to = null;

  -- A nullable legacy row is retained only when no current teacher owns the
  -- old assignment, so an administrator can link it later without data loss.
  insert into public.price_rules (teacher_id, grade_id, subject_id, student_price, teacher_fee_per_student, effective_from, effective_to, active)
  select null,
         grade_row.id,
         subject_row.id,
  ...
  join public.grades as grade_row on grade_row.stage_id = stage_row.id and grade_row.name = item ->> 'grade'
  join public.subjects as subject_row on subject_row.stage_id = stage_row.id and subject_row.name = item ->> 'subject'
  where nullif(item ->> 'teacherId', '') is null
  on conflict (grade_id, subject_id) where active = true and teacher_id is null do update
    set student_price = excluded.student_price,
        teacher_fee_per_student = excluded.teacher_fee_per_student,
        effective_to = null;
  ...
  where rule.active
    and not exists (
      ...
      join public.grades as grade_row on grade_row.stage_id = stage_row.id and grade_row.name = item ->> 'grade'
      join public.subjects as subject_row on subject_row.stage_id = stage_row.id and subject_row.name = item ->> 'subject'
      where grade_row.id = rule.grade_id
        and subject_row.id = rule.subject_id
        and coalesce(nullif(item ->> 'teacherId', ''), '') = coalesce(rule.teacher_id::text, '')
    );
...
-- Expand, version, and relationally synchronize the current operational state.
-- Every generated rule receives a numeric ID above the old maximum.
with expanded as (
  select state_row.id as state_id,
         price_item.value as rule,
         price_item.ordinality as rule_order,
         teacher_match.teacher_id,
         teacher_match.teacher_order,
         coalesce((
           select max(case when item ->> 'id' ~ '^[0-9]+$' then (item ->> 'id')::bigint end)
           from jsonb_array_elements(coalesce(state_row.data -> 'pricing', '[]'::jsonb)) as item
         ), 0) as max_rule_id
  from public.center_state as state_row
  cross join lateral jsonb_array_elements(coalesce(state_row.data -> 'pricing', '[]'::jsonb))
    with ordinality as price_item(value, ordinality)
  left join lateral (
    select teacher ->> 'id' as teacher_id,
           teacher_ordinality as teacher_order
    from jsonb_array_elements(coalesce(state_row.data -> 'teachers', '[]'::jsonb))
      with ordinality as teacher_item(teacher, teacher_ordinality)
    where nullif(price_item.value ->> 'teacherId', '') is null
      and coalesce((teacher ->> 'active')::boolean, true) = true
      and exists (
        select 1
        from jsonb_array_elements(coalesce(teacher -> 'assignments', '[]'::jsonb)) as assignment
        where assignment ->> 'stage' = price_item.value ->> 'stage'
          and assignment ->> 'grade' = price_item.value ->> 'grade'
          and assignment ->> 'subject' = price_item.value ->> 'subject'
      )
  ) as teacher_match on true
),
numbered as (
  select expanded.*,
         row_number() over (partition by state_id order by rule_order, teacher_order nulls last) as generated_order
  from expanded
),
candidates as (
  select numbered.*,
         case
           when nullif(rule ->> 'teacherId', '') is not null then rule
           when teacher_id is null then rule || jsonb_build_object('teacherId', '')
           else rule || jsonb_build_object(
             'teacherId', teacher_id,
             'id', (max_rule_id + generated_order)::text
           )
         end as linked_rule
  from numbered
),
  ...
  select candidates.*,
         row_number() over (
           partition by state_id,
                        coalesce(linked_rule ->> 'teacherId', ''),
                        linked_rule ->> 'stage',
                        linked_rule ->> 'grade',
                        linked_rule ->> 'subject'
           order by (nullif(rule ->> 'teacherId', '') is not null) desc, rule_order
         ) as key_order
  from candidates
),
rebuilt as (
  select state_id,
         coalesce(
           jsonb_agg(linked_rule order by rule_order, teacher_order nulls last),
           '[]'::jsonb
         ) as pricing
  from deduplicated
  where key_order = 1
...
update public.center_state as state_row
set data = jsonb_set(state_row.data, '{pricing}', rebuilt.pricing, true),
    version = state_row.version + 1,
    updated_at = now()
from rebuilt
where rebuilt.state_id = state_row.id;
